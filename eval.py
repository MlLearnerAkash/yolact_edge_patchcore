...
def prep_display(dets_out, img, h, w, undo_transform=True, class_color=False, mask_alpha=0.95):
    """
    ...
    if args.display_masks and cfg.eval_mask_branch:
        ...
        img_gpu2=((img_gpu_ * 1)*255).byte().cpu().numpy()
        cv2.imwrite("saghdsg.png",img_gpu2)
...
def prep_mask(dets_out, img, h, w, undo_transform=True, class_color=False, mask_alpha=0.95):
    ...
    if args.display_masks and cfg.eval_mask_branch:
        ...
        img_gpu_=img_gpu_ * inv_alph_masks.prod(dim=0)
        img_gpu2=((img_gpu_ * 1)*255).byte().cpu().numpy()
        cv2.imwrite("saghdsg_pr.png",img_gpu2)
    # Then draw the stuff that needs to be done on the cpu
    # Note, make sure this is a uint8 tensor or opencv will not anti alias text for whatever reason
    img_numpy = (img_gpu * 255).byte().cpu().numpy()
...
def evalimage(net:Yolact, path:str, save_path:str=None, detections:Detections=None, image_id=None):
    ...
    end_time=time.time()
    print("Time taken per image for prediction: ",end_time-st_time)
